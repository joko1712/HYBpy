function projhyb=genetare_semisymbolic_hybmod(projhyb, t, State, fState, rates, anninp, rann, ucontrol)
...
matlabFunction(anninp,DanninpDstate,DanninpDucontrol,'file',projhyb.mlm.xfun, 'vars',{t,[State],[ucontrol]})

%2) rates functions with sensitivities
DrDs = jacobian(rates,State);
DrDrann = jacobian(rates,rann);
matlabFunction(rates, DrDs, DrDrann,'file', projhyb.userdefun_parametric_kinetic, 'vars',{t,[State],[rann],[ucontrol]})
 
%3) ODEs function with Jacobain of ANN weights
DfDs = jacobian(fState,State);
DfDrann = jacobian(fState,rann);
matlabFunction(fState,DfDs,DfDrann,'file', projhyb.userdefun_parametric_odes,'vars',{t,[State],[rann],[ucontrol]})
