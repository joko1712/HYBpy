...
def hybodesolver(odesfun, controlfun, eventfun, t0, tf, state, jac=None, hess=None, w=None, batch=None, projhyb=None):
    ...
    if jac is None:  # only state
        while t < tf:
            ...
            ucontrol1 = controlfun(t, batch) if controlfun is not None else None
            k1_state = odesfun(t, state, None, None, w, ucontrol1, projhyb)

            ucontrol2 = controlfun(t + h / 2, batch) if controlfun is not None else None
            ...
            hl = h - h / 1e10
            ucontrol4 = controlfun(t + hl, batch) if controlfun is not None else None
